...
export const localProjects = {
  ...
  create(data) {
    ...
    const newProject = {
      ...
      assigned_officer_id: data.assigned_officer_id || data.officer_id || 'po_001',
      officer_id: data.assigned_officer_id || data.officer_id || 'po_001',
      field_personnel: [],
      tasks_count: 0,
      tasks_completed: 0,
      pending_approvals: 0,
  ...
  getStats() {
    ...
    return {
      total: items.length,
      active: items.filter((p) => p.status === 'active' || p.phase === 'ongoing').length,
      pipeline: items.filter((p) => p.status === 'pipeline' || p.phase === 'pipeline').length,
      completed: items.filter((p) => p.status === 'completed' || p.phase === 'completed').length,
