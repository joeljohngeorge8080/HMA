...
const LsgbAnalysisPage = () => {
  const [summary, setSummary] = useState({ totalBodies: 0, totalSanctioned: 0, totalWithdrawn: 0, remaining: 0, byPurpose: {}, byBody: {} })
    ...
    <>
      ...
      <div className="mb-4">
        ...
        <p className="text-body-secondary small mb-0">
          Overview of how much HMA has drawn from LSGB project funds across all bodies and expense categories.
        </p>
      </div>
      ...
      <CRow className="g-3 mb-4">
        {[
          ...
          { label: 'Total Sanctioned', value: fmt(summary.totalSanctioned), color: '#06d6a0', sub: 'across all bodies' },
          { label: 'Total Withdrawn', value: fmt(summary.totalWithdrawn), color: '#e74c3c', sub: 'by HMA' },
          { label: 'Remaining Balance', value: fmt(summary.remaining), color: summary.remaining < 0 ? '#e74c3c' : '#9b5de5', sub: 'available' },
        ].map((s) => (
          <CCol key={s.label} xs={6} md={3}>
            <CCard className="border-0 shadow-sm h-100" style={{ borderRadius: 12 }}>
              <CCardBody className="py-3 px-3">
                ...
                <div className="text-body-tertiary mt-1" style={{ fontSize: '0.72rem' }}>{s.sub}</div>
              </CCardBody>
            </CCard>
          </CCol>
        ))}
        ...
        <CCol xs={12} md={6}>
          <CCard className="border-0 shadow-sm h-100" style={{ borderRadius: 12 }}>
            <CCardHeader className="bg-transparent border-bottom py-3">
              <span className="fw-semibold small text-uppercase text-body-secondary" style={{ letterSpacing: '0.5px' }}>
            ...
            <CCardBody>
              {summary.totalSanctioned === 0 ? (
                <div className="text-center text-body-secondary py-3 small">No sanctioned amounts recorded yet.</div>
              ) : (
                <>
                  ...
                  <CProgress
                    ...
                    height={14}
                    color={utilisationPct > 90 ? 'danger' : utilisationPct > 70 ? 'warning' : 'primary'}
                    className="rounded-pill mb-3"
                  />
                  <div className="d-flex justify-content-between small text-body-secondary mt-2">
                    <span>Withdrawn: <strong className="text-dark">{fmt(summary.totalWithdrawn)}</strong></span>
                    <span>Of: <strong className="text-dark">{fmt(summary.totalSanctioned)}</strong></span>
                  </div>
                </>
              )}
              ...
              {/* By purpose */}
              <div className="mt-4">
                <div className="small fw-semibold text-body-secondary text-uppercase mb-3" style={{ letterSpacing: '0.5px' }}>By Purpose</div>
                    ...
                    <div key={purpose} className="mb-3">
                      <div className="d-flex justify-content-between mb-1 small">
                        <div className="d-flex align-items-center gap-2">
                          <CBadge color={PURPOSE_COLOR[purpose] || 'secondary'} shape="rounded-pill" style={{ fontSize: '0.6rem' }}>
                            {p}%
                          </CBadge>
                          <span>{purpose}</span>
                        </div>
                      ...
                      <CProgress value={p} height={6} color={PURPOSE_COLOR[purpose] || 'secondary'} className="rounded-pill" />
                    </div>
                  )
                })}
        ...
        <CCol xs={12} md={6}>
          <CCard className="border-0 shadow-sm h-100" style={{ borderRadius: 12 }}>
            ...
            <CCardBody className="p-0">
              {bodyRows.length === 0 ? (
                <div className="text-center text-body-secondary py-5 small">No LSGB bodies registered yet.</div>
              ) : (
                    ...
                    <div key={b.id}>
                      <div className="d-flex justify-content-between mb-1 small">
                        <div>
                          ...
                          <CBadge color="info" shape="rounded-pill" className="ms-2" style={{ fontSize: '0.6rem' }}>{b.body_type}</CBadge>
                        </div>
                        <span className="text-body-secondary">
                          {fmt(b.withdrawn)} / {b.sanctioned_amount ? fmt(b.sanctioned_amount) : 'N/A'}
                        </span>
                      </div>
                      {b.sanctioned_amount > 0 ? (
                        <CProgress
                          ...
                          height={6}
                          color={b.utilisationPct > 90 ? 'danger' : b.utilisationPct > 60 ? 'warning' : 'primary'}
                          className="rounded-pill"
                        />
                      ) : (
                        <div className="text-body-tertiary small">No sanctioned amount recorded</div>
                      )}
                    </div>
                  ))}
                </div>
      ...
      <CCard className="border-0 shadow-sm" style={{ borderRadius: 12 }}>
        <CCardHeader className="bg-transparent border-bottom py-3 d-flex align-items-center justify-content-between">
          ...
          <CBadge color="primary" shape="rounded-pill">{recentWithdrawals.length} shown</CBadge>
        </CCardHeader>
        <CCardBody className="p-0">
          {recentWithdrawals.length === 0 ? (
            <div className="text-center text-body-secondary py-5 small">No withdrawals recorded yet.</div>
          ) : (
              ...
              <CTable hover align="middle" className="mb-0" style={{ fontSize: '0.875rem' }}>
                ...
                <CTableBody>
                  {recentWithdrawals.map((w) => (
                    <CTableRow key={w.id}>
                      <CTableDataCell className="ps-3 fw-semibold">{w.lsgb_body_name || '—'}</CTableDataCell>
                      <CTableDataCell className="fw-bold" style={{ color: '#e74c3c' }}>{fmt(w.amount)}</CTableDataCell>
                      <CTableDataCell>
                        <CBadge color={PURPOSE_COLOR[w.purpose] || 'secondary'} shape="rounded-pill">{w.purpose}</CBadge>
                      </CTableDataCell>
                      <CTableDataCell className="small text-body-secondary">{fmtDate(w.withdrawal_date)}</CTableDataCell>
