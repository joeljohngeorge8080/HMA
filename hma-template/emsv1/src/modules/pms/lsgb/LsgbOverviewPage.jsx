...
const BodyForm = ({ form, setField, errors, lsgbOfficers }) => (
  <div>
    <div className="fw-semibold small text-body-secondary text-uppercase mb-3" style={{ letterSpacing: '0.5px' }}>
      Officer & Body — Required
    </div>
    <CRow className="g-3 mb-4">
      <CCol xs={12} md={6}>
        <CFormLabel className="small fw-semibold">LSGB Project Officer <span className="text-danger">*</span></CFormLabel>
        <CFormSelect value={form.officer_id} onChange={(e) => setField('officer_id', e.target.value)} invalid={!!errors.officer_id}>
          <option value="">— Select officer —</option>
          {lsgbOfficers.map((o) => (
            <option key={o.id} value={o.id}>{o.name} · {o.designation || 'Officer'}</option>
          ))}
        </CFormSelect>
        {errors.officer_id && <div className="text-danger small mt-1">{errors.officer_id}</div>}
      </CCol>
      <CCol xs={12} md={6}>
        <CFormLabel className="small fw-semibold">LSGB Body Type <span className="text-danger">*</span></CFormLabel>
        <CFormSelect value={form.body_type} onChange={(e) => setField('body_type', e.target.value)} invalid={!!errors.body_type}>
      ...
      <CCol xs={12}>
        <CFormLabel className="small fw-semibold">LSGB Body Name <span className="text-danger">*</span></CFormLabel>
        <CFormInput
    ...
    <CRow className="g-3 mb-4">
      <CCol xs={12} md={4}>
        ...
        <CFormInput placeholder="e.g., LSG/2025/001" value={form.sanction_number} onChange={(e) => setField('sanction_number', e.target.value)} />
      </CCol>
      <CCol xs={12} md={4}>
        <CFormLabel className="small fw-semibold">Sanction Date</CFormLabel>
        <CFormInput type="date" value={form.sanction_date} onChange={(e) => setField('sanction_date', e.target.value)} />
      </CCol>
      <CCol xs={12} md={4}>
        <CFormLabel className="small fw-semibold">Sanctioned Amount (₹)</CFormLabel>
        <CInputGroup>
          <CInputGroupText>₹</CInputGroupText>
          <CFormInput type="number" min="0" placeholder="0" value={form.sanctioned_amount} onChange={(e) => setField('sanctioned_amount', e.target.value)} />
        </CInputGroup>
      </CCol>
    </CRow>
    ...
    <CRow className="g-3 mb-4">
      <CCol xs={12} md={4}>
        ...
        <CFormInput placeholder="e.g., WO/2025/042" value={form.work_order_number} onChange={(e) => setField('work_order_number', e.target.value)} />
      </CCol>
      <CCol xs={12} md={4}>
        <CFormLabel className="small fw-semibold">Agreement Date</CFormLabel>
        <CFormInput type="date" value={form.agreement_date} onChange={(e) => setField('agreement_date', e.target.value)} />
      </CCol>
      <CCol xs={12} md={4}>
        <CFormLabel className="small fw-semibold">Validity Period</CFormLabel>
        <CFormInput placeholder="e.g., 2 years / Until 31-Mar-2027" value={form.validity_period} onChange={(e) => setField('validity_period', e.target.value)} />
      </CCol>
    </CRow>
    ...
    <CRow className="g-3">
      <CCol xs={12} md={6}>
        ...
        <CFormInput placeholder="Name of president / secretary" value={form.contact_person} onChange={(e) => setField('contact_person', e.target.value)} />
      </CCol>
      <CCol xs={12} md={6}>
        <CFormLabel className="small fw-semibold">Role</CFormLabel>
        <CFormSelect value={form.contact_role} onChange={(e) => setField('contact_role', e.target.value)}>
      ...
      <CCol xs={12} md={6}>
        ...
        <CFormInput placeholder="+91-XXXXX-XXXXX" value={form.contact_phone} onChange={(e) => setField('contact_phone', e.target.value)} />
      </CCol>
      <CCol xs={12} md={6}>
        <CFormLabel className="small fw-semibold">Office Address</CFormLabel>
        <CFormInput placeholder="Office address" value={form.office_address} onChange={(e) => setField('office_address', e.target.value)} />
      </CCol>
    </CRow>
  </div>
)
...
const BodyDetailPanel = ({ body, officerMap }) => {
  ...
  return (
    <div className="px-4 py-3 bg-body-secondary border-top">
      <CRow className="g-3">
        <CCol xs={12} md={4}>
          <div className="small fw-semibold text-body-secondary text-uppercase mb-2">Sanction Details</div>
          {[
            ...
            <div key={l} className="d-flex gap-2 mb-1" style={{ fontSize: '0.82rem' }}>
              <span className="text-body-secondary flex-shrink-0" style={{ minWidth: 120 }}>{l}</span>
              <span className="fw-medium">{v || '—'}</span>
            </div>
          ))}
        </CCol>
        <CCol xs={12} md={4}>
          <div className="small fw-semibold text-body-secondary text-uppercase mb-2">Work Order</div>
          {[
        ...
        <CCol xs={12} md={4}>
          <div className="small fw-semibold text-body-secondary text-uppercase mb-2">Contact & Officer</div>
          {[
...
const LsgbOverviewPage = () => {
  ...
  const reload = useCallback(() => {
    ...
    const map = {}
    officers.forEach((o) => { map[o.id] = o })
    setOfficerMap(map)
  }, [search, typeFilter])

  useEffect(() => { reload() }, [reload])

  const setAddField = (f, v) => { setAddForm((p) => ({ ...p, [f]: v })); setAddErrors((p) => ({ ...p, [f]: null })) }
  const setEditField = (f, v) => { setEditForm((p) => ({ ...p, [f]: v })); setEditErrors((p) => ({ ...p, [f]: null })) }
  ...
  const handleAdd = () => {
    ...
    localLsgb.createBody({ ...addForm, sanctioned_amount: parseFloat(addForm.sanctioned_amount) || 0 })
    setAddOpen(false)
  ...
  const handleEdit = () => {
    ...
    localLsgb.updateBody(editBody.id, { ...editForm, sanctioned_amount: parseFloat(editForm.sanctioned_amount) || 0 })
    setEditBody(null)
    reload()
  }
    ...
    <>
      ...
      <div className="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-body-secondary small mb-0">
            Manage Local Self Government Body details and track project officers assigned to LSGB work.
          </p>
        </div>
        <CButton color="primary" onClick={() => { setAddForm(EMPTY_BODY); setAddErrors({}); setAddOpen(true) }}>
      ...
      <CRow className="g-3 mb-4">
        {[
          ...
          { label: 'Total Sanctioned', value: totalSanctioned ? fmt(totalSanctioned) : '—', color: '#9b5de5' },
        ].map((s) => (
      ...
      {lsgbOfficers.length === 0 && (
        <CAlert color="warning" className="mb-4 small">
          ...
          <a href="/pms/project-teams/officers" className="alert-link">Project Officers</a> and set the Project Type to LSGB when adding or editing an officer.
        </CAlert>
      )}
      ...
      <CCard className="border-0 shadow-sm" style={{ borderRadius: 12 }}>
        {bodies.length === 0 ? (
          <CCardBody className="text-center py-5 text-body-secondary">
            <CIcon icon={cilBuilding} style={{ width: 48, height: 48, opacity: 0.3 }} className="mb-2 d-block mx-auto" />
            <div className="small">{search || typeFilter ? 'No LSGB bodies match your search.' : 'No LSGB bodies added yet. Click "Add LSGB Body" to get started.'}</div>
          </CCardBody>
        ) : (
                ...
                {bodies.map((body, i) => (
                  <React.Fragment key={body.id}>
                    ...
                    >
                      ...
                      <CTableDataCell>
                        <CBadge color="info" shape="rounded-pill">{body.body_type}</CBadge>
                      </CTableDataCell>
                      <CTableDataCell className="small">
                        {officerMap[body.officer_id]?.name || <span className="text-body-secondary">—</span>}
                      </CTableDataCell>
                      <CTableDataCell className="fw-semibold" style={{ color: '#4361ee' }}>
                        {body.sanctioned_amount ? fmt(body.sanctioned_amount) : <span className="text-body-secondary small">—</span>}
                      </CTableDataCell>
                      ...
                      <CTableDataCell className="text-end pe-3" onClick={(e) => e.stopPropagation()}>
                        <CButton color="secondary" variant="ghost" size="sm" className="me-1" onClick={() => openEdit(body)}>
                          <CIcon icon={cilPencil} />
                        </CButton>
                        <CButton color="danger" variant="ghost" size="sm" onClick={() => handleRemove(body.id)}>
                          <CIcon icon={cilTrash} />
                        </CButton>
                        <CIcon
                          icon={expandedId === body.id ? cilChevronTop : cilChevronBottom}
                          style={{ width: 14, height: 14, color: 'var(--cui-body-secondary-color)', marginLeft: 8 }}
                          onClick={(e) => { e.stopPropagation(); setExpandedId(expandedId === body.id ? null : body.id) }}
                        />
                      </CTableDataCell>
                    </CTableRow>
      ...
      {/* Add Modal */}
      <CModal visible={addOpen} onClose={() => setAddOpen(false)} alignment="center" size="lg" scrollable>
        <CModalHeader closeButton>
          <CModalTitle><CIcon icon={cilBuilding} className="me-2 text-primary" />Add LSGB Body</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <BodyForm form={addForm} setField={setAddField} errors={addErrors} lsgbOfficers={lsgbOfficers} />
        </CModalBody>
        <CModalFooter>
          <CButton color="secondary" variant="ghost" onClick={() => setAddOpen(false)}>Cancel</CButton>
          <CButton color="primary" onClick={handleAdd}>
            <CIcon icon={cilPlus} className="me-1" />Save LSGB Body
          </CButton>
        </CModalFooter>
      </CModal>

      {/* Edit Modal */}
      <CModal visible={!!editBody} onClose={() => setEditBody(null)} alignment="center" size="lg" scrollable>
        <CModalHeader closeButton>
          <CModalTitle><CIcon icon={cilPencil} className="me-2 text-success" />Edit — {editBody?.body_name}</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <BodyForm form={editForm} setField={setEditField} errors={editErrors} lsgbOfficers={lsgbOfficers} />
        </CModalBody>
        <CModalFooter>
          <CButton color="secondary" variant="ghost" onClick={() => setEditBody(null)}>Cancel</CButton>
          <CButton color="success" onClick={handleEdit}>Save Changes</CButton>
        </CModalFooter>
      </CModal>
    </>
  )
