...
import { cilPlus, cilPencil, cilTrash, cilMoney } from '@coreui/icons'
        ...
        <CRow className="g-3">
          <CCol xs={12}>
            <CFormLabel className="small fw-semibold">
              LSGB Body <span className="text-danger">*</span>
            </CFormLabel>
            <CFormSelect
              value={form.lsgb_body_id}
              onChange={(e) => set('lsgb_body_id', e.target.value)}
              invalid={!!errors.lsgb_body_id}
            >
              <option value="">— Select LSGB body —</option>
              {bodies.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.body_name} ({b.body_type})
                </option>
              ))}
            </CFormSelect>
            {errors.lsgb_body_id && (
              <div className="text-danger small mt-1">{errors.lsgb_body_id}</div>
            )}
            {selected?.sanctioned_amount > 0 && (
              <div className="small text-body-secondary mt-1">
                Sanctioned: <strong>{fmt(selected.sanctioned_amount)}</strong>
              </div>
            )}
          </CCol>
          ...
          <CCol xs={12} md={6}>
            <CFormLabel className="small fw-semibold">
              Amount Withdrawn (₹) <span className="text-danger">*</span>
            </CFormLabel>
            <CInputGroup>
          ...
          <CCol xs={12} md={6}>
            ...
            <CFormInput
              ...
              invalid={!!errors.withdrawal_date}
            />
            {errors.withdrawal_date && (
              <div className="text-danger small mt-1">{errors.withdrawal_date}</div>
            )}
          </CCol>

          <CCol xs={12}>
            <CFormLabel className="small fw-semibold">
              Purpose / Used For <span className="text-danger">*</span>
            </CFormLabel>
            <CFormSelect
              value={form.purpose}
...
const LsgbFundsPage = () => {
  ...
  const openEdit = (w) => {
    setEditTarget({ ...w, amount: String(w.amount) })
    setModalOpen(true)
  }
  ...
  return (
    <>
      ...
      <div className="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-body-secondary small mb-0">
            Track how much HMA has drawn from LSGB project funds — used for core, HR, and other
      ...
      <CRow className="g-3 mb-4">
        {[
          {
            label: 'Total Sanctioned (all bodies)',
          ...
          { label: 'Remaining Fund', value: fmt(summary.remaining), color: '#06d6a0' },
          {
            ...
            value: localLsgb.listWithdrawals().length,
            color: '#9b5de5',
          },
        ].map((s) => (
          ...
          <CCardBody className="py-3">
            <div className="d-flex justify-content-between mb-1 small">
              ...
              <span className="text-body-secondary">
                ...
                {Math.min(
                  100,
                  Math.round((summary.totalWithdrawn / summary.totalSanctioned) * 100),
                )}
                %)
              </span>
            </div>
            <CProgress
              ...
              height={10}
              color={
                summary.totalWithdrawn / summary.totalSanctioned > 0.9
                  ? 'danger'
                  : summary.totalWithdrawn / summary.totalSanctioned > 0.6
                    ? 'warning'
                    : 'primary'
              }
      ...
      {bodies.length === 0 && (
        <CAlert color="info" className="mb-4 small">
          No LSGB bodies found.{' '}
          <a href="/pms/lsgb/overview" className="alert-link">
            Add an LSGB body
          </a>{' '}
          first before recording withdrawals.
        </CAlert>
      )}
          ...
          <CCardBody className="text-center py-5 text-body-secondary">
            ...
            <div className="small">
              {filterBody || filterPurpose
                ...
                : 'No fund withdrawals recorded yet.'}
            </div>
          </CCardBody>
        ) : (
